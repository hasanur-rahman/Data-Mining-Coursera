...
float pl2_ranker::score_one(const index::score_data& sd) {
  ...
  **/
  ...
  float first_part_result = 1.0 / (tfn + 1);
  float sec_part_result = tfn * log2(tfn / lambda);
  float thr_part_result = (lambda - tfn + 1.0 / (12.0 * tfn)) * log2(exp(1));
  float fourth_part_result = 0.5 * log2(2 * pi * tfn);

  return first_part_result * (sec_part_result + thr_part_result + fourth_part_result);  // Change 0 to the final score you calculated
}
